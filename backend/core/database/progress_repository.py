...
from core.models.progress import (
    LearningProgress,
    StudySession,
    ConceptRelationship,
    LearningPath,
...
class ProgressRepository:
    ...
    def track_concept_interaction(
        ...
        try:
            ...
            # Update last studied
            progress.last_studied = get_current_time()
    ...
    def get_study_recommendations(
        ...
        try:
            ...
            for progress in progress_list[:limit]:
                ...
                elif (get_current_time() - progress.last_studied).days > 7:
    ...
    def start_study_session(
        ...
        try:
            ...
            if self.db:
                ...
                return f"session_{user_id}_{get_current_time().timestamp()}"
    ...
    def get_study_sessions(
        ...
        try:
            if self.db:
                cutoff = get_current_time() - timedelta(days=days_back)
    ...
    def update_memory_context(
        ...
        try:
            ...
            context.last_updated = get_current_time()
    ...
    def get_user_progress_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for prog in all_progress:
                mastery_dist[prog.mastery_level.value] += 1
            
            # Count mastered concepts (proficient + expert)
            concepts_mastered = mastery_dist["proficient"] + mastery_dist["expert"]
            
            # Get study sessions for time calculation
            sessions = self.get_study_sessions(user_id, days_back=365)
            total_minutes = sum(s.duration_minutes for s in sessions)
            total_hours = total_minutes / 60.0
            
            # Calculate study streak using proper method
            streak_days = self.calculate_study_streak(user_id)
            ...
            return {
                ...
                "mastery_distribution": {
                    "novice": 0, "learning": 0, "competent": 0,
                    "proficient": 0, "expert": 0
                },
                "total_hours": 0.0,
                "streak_days": 0
            }
    
    def get_recent_concepts(self, user_id: str, limit: int = 5) -> List[LearningProgress]:
        """
        ...
        Returns:
            List of recent learning progress records
        """
        try:
            all_progress = self.get_user_progress(user_id)
            # Sort by last_studied descending
            all_progress.sort(key=lambda p: p.last_studied, reverse=True)
    ...
    def log_study_session(
        ...
        """
        Log a study session for dashboard stats.
        
        THIS IS THE KEY METHOD FOR DASHBOARD STATS!
        """
        try:
            from core.models.progress import StudySession
            ...
            if self.db:
                ...
                logger.info(f"Study session logged: {user_id}, {duration_minutes:.1f}min, {activity_type}")
                return True
    ...
    def calculate_study_streak(self, user_id: str) -> int:
        ...
        try:
            ...
            study_dates = sorted(set(s.session_start.date() for s in sessions), reverse=True)
